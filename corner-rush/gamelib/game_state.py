...
class GameState:
    ...
    def mod_number_affordable(self, unit_type, MP_budget=0, SP_budget=0):
        ...
        if costs[MP] > 0:
            return math.floor(MP_budget / costs[MP])
        elif costs[SP] > 0:
            return math.floor(SP_budget / costs[SP])
        else:
    ...
    def attempt_spawn_return_cost(self, unit, location, num=1):
        num_spawned = int(self.attempt_spawn(unit, location, num) or 0)
        return num_spawned * max(self.type_cost(unit)[SP], self.type_cost(unit)[MP])
    ...
    def attempt_upgrade_return_cost(self, locations):
        num_spawned = int(self.attempt_upgrade(locations) or 0)
        return num_spawned * self.type_cost(UPGRADE, True)[SP]
