...
class GameState:
        ...
        for i, unit_types in enumerate(units):
            for uinfo in unit_types:
                ...
                if unit_type == REMOVE:
                    ...
                    unit = GameUnit(unit_type, self.config, player_number, hp, x, y, creationIndex)
                    if player_number == 0:
                        self.p1_units.append(unit)
                    else:
                        self.p2_units.append(unit)
    ...
    def getUnitAtLocation(self, x, y):
        self.game_map.__getitem__((x,y))
